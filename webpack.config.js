...
var plugins = [
  new webpack.BannerPlugin("create by laiyangde"),
    new HtmlWebpackPlugin({                        //根据模板插入css/js等生成最终HTML
        ...
        template:'./projectSrc/'+project+'/index.html',    //html模板路径
    ...
    //     from: __dirname + '/projectSrc/project1/images',
    //     to:   __dirname + "/触屏/"+title,
    // }]),
...
module.exports = {
  ...
  // 对象：entry: {index: '...', login : [...] }：如果传入Object，则会生成多个入口打包文件，key是chunk名，value可以是字符串或数组。
    entry:  __dirname + "/projectSrc/"+project+"/js/index.js",

    // 配置输出目录
    output: {
      path: __dirname + "/触屏/"+title,//指定输出文件路径，通常设置为__dirname + ‘/build’,
